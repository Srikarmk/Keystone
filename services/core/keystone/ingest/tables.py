...
#: Table cells are read with `unwrap_numeric_math` from the ingest layer, because
#: `strip_markup` deletes `$...$` — right for prose that has to appear verbatim on a
#: page, wrong for a cell, where the common case is a number someone typeset in
#: maths mode. GoogLeNet writes its whole top-5 error column as `$7.89\%$`, so every
#: value in it was dropped, and a paper citing those figures was then reported as
#: attributing numbers the source "does not report".
...
def build_table(source: TableSource) -> Table:
    ...
    for r, row in enumerate(grid):
        ...
        for c, (text, emphasised) in enumerate(row):
            plain = strip_markup(unwrap_numeric_math(text)).strip()
            if not plain:
                continue
            cells.append(
                TableCell(
...
def _count_header_rows(grid: list[list[tuple[str, bool]]]) -> int:
    ...
    count = 0
    for row in grid[:3]:
        values = [strip_markup(unwrap_numeric_math(text)).strip() for text, _ in row]
...
def _leaf_headers(grid: list[list[tuple[str, bool]]], header_rows: int) -> dict[int, str]:
    ...
    return {
        index: strip_markup(unwrap_numeric_math(text)).strip()
        for index, (text, _) in enumerate(leaf)
        if strip_markup(unwrap_numeric_math(text)).strip()
    }
